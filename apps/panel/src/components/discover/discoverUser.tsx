...
import { useFetchSports } from "@/features/sports/hook";
...
export const DiscoverUser = (props: DiscoverUserProps) => {
  ...
  const userTeam = props.teams.find((team) =>
    team.userIds.includes(props.user.id)
  );
  const userMatches = props.matches.filter(
    (match) =>
      match.leftTeamId === userTeam?.id || match.rightTeamId === userTeam?.id
  );
  ...
  const userMatchSports = sports.filter((sport) =>
    userMatchSportIds.includes(sport.id)
  );
          ...
          <Container maxWidth={"xl"}>
            <Stack spacing={1}>
              ...
              >
                ...
                ></Box>
                <Card
                  sx={{ backgroundColor: `${theme.palette.secondary.dark}80` }}
                >
                  <Stack
                    direction={"row"}
                    px={2}
