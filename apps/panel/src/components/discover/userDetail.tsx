...
import { HiClock, HiFlag, HiMapPin, HiUser } from "react-icons/hi2";
import { ThemeProvider, useTheme } from "@mui/material/styles";
import { useFetchUsers } from "@/features/users/hook";
import { useFetchTeams } from "@/features/teams/hook";
import { useFetchLocations } from "@/features/locations/hook";
...
export const UserDetail = (props: UserDetailProps) => {
  ...
  //  get left team
  const leftTeamModel = teams.find(
    (team) => team.id === props.match.leftTeamId
  );
  // get right team
  const rightTeamModel = teams.find(
    (team) => team.id === props.match.rightTeamId
  );
  // Get judge team
  const judgeTeam = teams.find((team) => team.id === props.match.judgeTeamId);
  //  get time and location
  const formattedTime = new Date(props.match.startAt).toLocaleTimeString(
    "ja-JP",
    {
      hour: "2-digit",
      minute: "2-digit",
  ...
  const locationModel = locations.find(
    (location) => location.id === props.match.locationId
  );
      ...
      >
        <Container maxWidth={"xl"}>
          <Stack spacing={1}>
            ...
            {users
              .filter((user) =>
                user.teamIds.includes(Number(leftTeamModel?.id))
              )
              .map((user) => {
            ...
            {users
              .filter((user) =>
                user.teamIds.includes(Number(rightTeamModel?.id))
              )
              .map((user) => {
                const image = `${process.env.NEXT_PUBLIC_API_URL}/images/${user?.pictureId}/file`;
                return (
