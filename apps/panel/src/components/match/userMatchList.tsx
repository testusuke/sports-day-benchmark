...
export const UserMatchList = (props: UserMatchListProps) => {
  ...
  const filteredMatches = matches.filter(
    (match) =>
      match.leftTeamId === userTeam?.id ||
      match.rightTeamId === userTeam?.id ||
      match.judgeTeamId === userTeam?.id
  );
  const maxLeftScore = Math.max.apply(
    Math,
    filteredMatches.map((match) => match.leftScore)
  );
  const maxRightScore = Math.max.apply(
    Math,
    filteredMatches.map((match) => match.rightScore)
  );
