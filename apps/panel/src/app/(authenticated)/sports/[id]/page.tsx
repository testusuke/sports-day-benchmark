...
import { useFetchSport, useFetchSportGames } from "@/features/sports/hook";
import { useFetchTeams } from "@/features/teams/hook";
import { useFetchLocations } from "@/features/locations/hook";
import { useFetchMatches } from "@/features/matches/hook";
...
export default function Page({ params }: { params: { id: string } }) {
  ...
  const {
    sport,
    isFetching: isSportFetching,
    refresh: refreshSport,
  } = useFetchSport(+params.id);
  const {
    games,
    isFetching: isGameFetching,
    refresh: refreshGame,
  } = useFetchSportGames(+params.id, true);
  const {
    matches,
  ...
  const myTeams = teams.filter((team) =>
    team.userIds.includes(Number(user?.id))
  );
  const myGames = games.filter((game) =>
    myTeams.some((team) => team.enteredGameIds.includes(game.id))
  );
  myGames.sort((a, b) => b.weight - a.weight);
  const myGame = myGames[0];
  const myTeam = myTeams.find((team) =>
    team.enteredGameIds.includes(myGame?.id)
  );
  //  state
                ...
                <CircleContainer>
                  <Container maxWidth={"xl"}>
                    ...
                    >
                      ...
                      >
                        {!sport.iconId && (
                          <HiOutlineExclamationTriangle fontSize={"30px"} />
                        )}
                      </Avatar>
                      <Typography
                        sx={{
                            ...
                            >
                              ...
                              >
                                ...
                                >
                                  ...
                                  <Typography>{myTeam?.name}</Typography>
                                </Stack>
                                <SvgIcon>
                            ...
                            >
                              ...
                              >
                                ...
                                >
                                  ...
                                  <Typography>{myGame?.name}</Typography>
                                </Stack>
                                <SvgIcon>
